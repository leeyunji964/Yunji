...
mpg_rg <- get_googlemap( center = region, maptype = "roadmap")
ggmap(mpg_rg)
...
traffic <- read.csv("D:/workR/Report.csv"); head(traffic)
traffic <- traffic[8:82,2:4]; traffic
# traffic <- subset( traffic, 시도 == '서울' )

traffic <- subset( traffic, traffic$기준년도 == '발생건수'); traffic
str(traffic)
traffic <- traffic[,-2]; traffic
str(traffic)

st <- as.character(traffic$시군구); class(st)
...
city <- as.character( jtraffic$시군구 ); city

jcnt <- as.numeric( as.character(jtraffic$X2018) ); jcnt
