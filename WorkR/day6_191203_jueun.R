...
#다중변수 자료 탐색
#두 변수 사이의 산점도
#산점도(scatter plot)-2변수로 구성된 자료의 분포를 알아보는 관측값들의 분포를 2 변수 사이의 관계 파악
#
...
#방법2 흐름지정
plot(mpg~wt,data=mtcars,main='중량-연비 그래프', xlab='중량',ylab='연비',col='red',pch=19)
#방법3 
plot(mtcars[,c("wt","mpg")],main='중량-연비 그래프', xlab='중량',ylab='연비',col='red',pch=19)
#방법4 직접지정
plot(mtcars$wt,mtcars$mpg,main='중량-연비 그래프', xlab='중량',ylab='연비',col='red',pch=19)
...
#그룹정보가 있는 두변수의 산점도
iris.2 <- iris[,3:4]
point <- as.numeric(iris$Species) #범주형=그룹 
iris$Species
point
...
#상관분석
beers <- c(5,2,9,8,3,7,3,5,3,5) #맥주 몇병
bal <- c(0.1,0.03,0.19,0.12,0.04,0.0095,0.07,0.06,0.02,0.05) #알콜농도
...
plot(month,late, main='지각생 통계', type='b',lty=1,lwd=1, xlab='Month', ylab='late cnt')
plot(month,late, main='지각생 통계', type='o',lty=1,lwd=1, xlab='Month', ylab='late cnt')
plot(month,late, main='지각생 통계', type='s',lty=1,lwd=1, xlab='Month', ylab='late cnt')
#왜 5월달부터 지각생이 많아졌을까? 원인분석 내부/ 다른 데이터 추가적으로 필요한지 분석..

#복수의 선 그래프
late1 <- c(5,8,7,9,4,6,12,13,8,6,6,4)
late2 <- c(4,6,5,8,7,8,10,11,6,5,7,3)
plot(month,late1, main='지각생 통계', type='b',lty=1,col='red', xlab='Month', ylab='late cnt', ylim=c(1,15))
